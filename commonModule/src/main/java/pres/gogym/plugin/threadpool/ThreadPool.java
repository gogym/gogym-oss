/*
 * 文件名：ThreadPool2.java 版权：Copyright by www.poly.com 描述： 修改人：gogym 修改时间：2018年7月10日 跟踪单号： 修改单号：
 * 修改内容：
 */
...
public class ThreadPool {
	 ...
	 *            线程池类型
	 * @param corePoolSize
	 *            只对Fixed和Scheduled线程池起效
	 */
	public ThreadPool(final int type, final int corePoolSize) {
		// 构造有定时功能的线程池
		// ThreadPoolExecutor(corePoolSize, Integer.MAX_VALUE, 10L,
		...
		case FixedThread:
			// 构造一个固定线程数目的线程池
			// ThreadPoolExecutor(corePoolSize, corePoolSize, 0L,
			// TimeUnit.MILLISECONDS, new
	 ...
	 * @see
	 */
	public void shutDownAwaitTermination(final long awaitTime) {
	 ...
	 * 执行给定的任务
	 * <p>
	 * 当所有任务完成时，返回保持任务状态和结果的Future列表。 返回列表的所有元素的{@link Future#isDone}为
	 * {@code true}。 注意，可以正常地或通过抛出异常来终止已完成任务。 如果正在进行此操作时修改了给定的
	 ...
	 * 执行给定的任务
	 * <p>
	 * 当所有任务完成或超时期满时(无论哪个首先发生)，返回保持任务状态和结果的Future列表。 返回列表的所有元素的
	 * {@link Future#isDone}为 {@code true}。 一旦返回后，即取消尚未完成的任务。
	...
	public ScheduledFuture<?> scheduleWithFixedRate(final Runnable command,
		...
		return scheduleExec.scheduleAtFixedRate(command, initialDelay, period,
				unit);
	}
	...
	public ScheduledFuture<?> scheduleWithFixedDelay(final Runnable command,
		...
		return scheduleExec.scheduleWithFixedDelay(command, initialDelay,
				delay, unit);
	}
